...
function addItem (elements) {
  elements.form.addEventListener('click', async event => {
    if (event.target.className === 'submit') {
      ...
      if (todoData) modifyTodo('http://localhost:3000/todos', todoData)
...
function checkBoxClick (elements) {
  elements.ul.addEventListener('click', async event => {
    if (event.target.type === 'checkbox') {
      ...
      displayTodos(data, elements)
      modifyTodo('http://localhost:3000/todos/checkBox', { parentId, complete: event.target.checked })
    }
  })
}
  ...
  elements.ul.addEventListener('focusout', async event => {
    if (event.target.className === 'textArea') {
      ...
      if (elements.textAreaInput) {
        ...
        modifyTodo('http://localhost:3000/todos/updateText', { parentId, text: elements.textAreaInput })
      } else {
        data = data.map(todo => {
          if (parentId === todo.id) {
            todo.text = value
        ...
        modifyTodo('http://localhost:3000/todos/updateText', { parentId, text: value })
      }
      displayTodos(data, elements)
      elements.textAreaInput = ''
    }
...
function saveNote (id, data, elements) {
  elements.saveBtn.addEventListener('click', event => {
    ...
    if (notes[0] === '') {
      ...
      modifyTodo('http://localhost:3000/todos/updateNote', { id, noteText: elements.popUpBox.value, note: true })
    } else {
      modifyTodo('http://localhost:3000/todos/updateNote', { id, noteText: elements.popUpBox.value, note: true })
...
function saveDate (id, data, elements) {
  elements.saveBtn.addEventListener('click', event => {
    ...
    if (dates[0] === '') {
      ...
      modifyTodo('http://localhost:3000/todos/updateDate', { id, date: true, displayDate: val })
    } else {
      displayTodos(data, elements)
      modifyTodo('http://localhost:3000/todos/updateDate', { id, date: true, displayDate: val })
...
function changePriority (id, data, elements, eventName, btnName, arr) {
  btnName.addEventListener(eventName, event => {
    ...
    displayTodos(data, elements)
    modifyTodo('http://localhost:3000/todos/updatePriority', { id, priority: true, lowPriority: arr[0], mediumPriority: arr[1], highPriority: arr[2] })
  })
}
